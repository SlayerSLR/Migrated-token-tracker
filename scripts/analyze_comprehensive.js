...
const run = async () => {
    try {
        ...
        // Only fetch tokens that have fired an alert
        const tokens = await Token.find({ firstAlertMarketCap: { $exists: true, $ne: null } }).lean();
        console.log(`Analyzing ${tokens.length} alerted tokens...`);
        ...
        const enrichedTokens = await Promise.all(tokens.map(async (t) => {
            ...
            // No fallback to candles anymore, we only want alerts!
        ...
        let stats = {
            total: 0,
            winners: 0, // current > 0%
            mooners: 0, // current > 100%
            peakWinners: 0, // peak > 20% (Win Rate)
            peakMooners: 0, // peak > 100% (2x Hit Rate)
            rugged: 0, // current <-90%
            totalCurrentGain: 0,
            totalPeakGain: 0
        };
        ...
        for (const t of validTokens) {
            ...
            if (peakGain > 20) stats.peakWinners++;
        ...
        console.log(`Win Rate(Peak>20):${stats.peakWinners} (${(stats.peakWinners / stats.total * 100).toFixed(1)}%)`);
        console.log(`2x Hit Rate(>100):${stats.peakMooners} (${(stats.peakMooners / stats.total * 100).toFixed(1)}%)`);
